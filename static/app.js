...
async function safeJson(url, options) {
  ...
  try {
    ...
    throw new Error(text.slice(0, 200) || "服务器返回非JSON响应");
  }
  if (!response.ok) {
...
async function createSession() {
  ...
  const data = await safeJson("/api/session", {
    method: "POST",
...
async function analyzeFrame() {
  ...
  const result = await safeJson("/api/analyze", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
async function uploadVideo(file) {
  ...
  const result = await safeJson("/api/analyze-video", {
    method: "POST",
    body: formData
  });
  updateUI(result, 0, 0);
