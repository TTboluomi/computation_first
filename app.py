...
DB_PATH = BASE_DIR / "deepfake_platform.db"
...
class FaceDetector:
    ...
    def detect(self, frame: np.ndarray) -> Tuple[Tuple[int, int, int, int], bool]:
        ...
        if result.detections:
            ...
            x = int(max(0, box.xmin * width))
            y = int(max(0, box.ymin * height))
            w = int(min(width - x, box.width * width))
            h = int(min(height - y, box.height * height))
            if w > 0 and h > 0:
                return (x, y, w, h), True
...
# ----------------------------------------------------------------
# 2. FaceLandmarkDetector  –  MediaPipe FaceMesh 468 landmarks
# ----------------------------------------------------------------
...
def compute_optical_flow_score(
    ...
    if prev_gray is None or prev_landmarks is None:
        return 0.5, 0.0, curr_landmarks[lk_indices]
...
_tcn_model: Optional[TemporalCNN] = None
_tcn_ready = False


def get_tcn_model() -> Optional[TemporalCNN]:
